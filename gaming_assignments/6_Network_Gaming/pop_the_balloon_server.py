        ...
        if len(in_msgs) > 0:
            ...
            if len(received_list) > 0:
                if received_list[0] == "click" and len(received_list) >= 3:
                    userclickx,userclicky = received_list[1],received_list[2]
                    if userclickx.isnumeric() and userclicky.isnumeric():
                        if clicked_image (int(userclickx),int(userclicky), b_x, b_y, b_w, b_h):
